...
class Generator {
	...
	public function generate_random_users(Int $amount = 10) {
		for($i = 0; $i < $amount; $i++) {
			...
			$user -> profile() -> first_name = $this -> gen_name();
			$user -> profile() -> second_name = $this -> gen_name();
			$user -> profile() -> phone_number = "+38063" . rand(1000000, 9999999);
	...
	protected function gen_word(Int $len = 5, String $lang = "en") {
		...
		$letters_total = [
			"ru" => strlen($letters["ru"]) - 1,
			"en" => strlen($letters["en"]) - 1,
		];
